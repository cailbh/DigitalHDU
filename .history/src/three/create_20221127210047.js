...
export default {
  ...
  Animate:(controls,scene,camera,renderer,composer,finalComposer)=>{
    ...
    function calcHeight() {
      ...
      // 扫描动态效果实现
      uHeight += length / scanConfig.during / 1;
    ...
    function boxScan(obj){
      const uperVertext = `
          ...
          void main()
          {
            ...
            float strengthg= mod((angle + 3.14) / 6.28 * 16.0, 1.0);
            float strengthb = mod((angle + 3.14) / 6.28 * 24.0, 1.0);
            //模型的基础颜色
      ...
      scanConfig.start = -(boundingBox.max.y-boundingBox.min.y)/2+0.5||0;
      scanConfig.end = boundingBox.max.y-0.5 || 0;
      uTime += 0.1;
