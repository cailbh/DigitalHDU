...
export default {
		...
		mounted() {
			...
			this.height = this.$refs.stdChart.clientHeight
			this.width = this.$refs.stdChart.clientWidth
			this.initSvg()
			this.getData();
			this.draw();
		},
		methods: {
			initSvg(){
				...
				var svg =  d3.select('#studentsContain')
					.append("svg")
					.attr("id", "studentsSVG")
					.attr("width", svgWidth)
			...
			draw() {
				...
				var svg =  d3.select('#studentsSVG')
				svg.select('#studentsG').remove()
				var g = svg.append("g")
					.attr("id", "studentsG")
					.attr("width", svgWidth)
					.attr("height", svgHeight)
