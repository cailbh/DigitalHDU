...
export default {
  ...
  watch: {
    type(val){
      ...
      if(_this.sceneType != val){
        if("Living" == val){
        _this.animateCameraFun(camera.position,controls.target,new THREE.Vector3(150,100,150),new THREE.Vector3(0,0,0));
        ...
        _this.animateCameraFun(camera.position,controls.target,new THREE.Vector3(180,130,180),new THREE.Vector3(0,0,0));
    ...
    showLabelState(val){
      const _this = this;
      let scene = _this.scene;
      let sceneType = this.type;
      let groupLabel = scene.getObjectByName("label");
      scene.removeChild(groupLabel)
      console.log(12312312,groupLabel);
      // if(val){
      //     scene.traverse(function(obj){
      //       let objName = obj.name.split("/");
      //       console.log(filenames[sceneType].indexOf(objName[objName.length-1]),objName[objName.length-1])
      //       if((filenames[sceneType].indexOf(objName[objName.length-1])>0)){
      //         _this.showLabel(scene,obj,objName[objName.length-1])
      //       }
      //     })
      //   }
      //   else{
      ...
      if (intersects.length) {
        ...
        scene.add(wallMesh)
          ...
          // camera.position.set(worldPosition.x + 100, worldPosition.y + 100,worldPosition.z + 100);
            ////---------------------------悬浮图表
            if(_this.showLabelState == 0){this.showLabel(scene,obj,objName[objName.length-1]);}
            ////----------------------------------video
  ...
  created() {
    ...
    cubeA.position.set( 100, 0, 100 );

    var cubeB = new THREE.Mesh( geometry, material );
    cubeB.position.set( -100, 0, 100 );
        var group = new THREE.Group();
    group.add( cubeA );
    group.add( cubeB );
    group.name="label";
    sceneTeaching.add(group);
