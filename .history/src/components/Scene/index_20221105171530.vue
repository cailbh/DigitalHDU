        ...
        intersects[0].object.getWorldPosition(worldPosition); // 将点中的3D模型坐标记录到worldPosition中
          ...
          worldPosition.z +=(box.max.z - box.min.z)/2
            ...
            .then((dataUrl) => {
              ...
              scene.add(sprite);
                        let boxs= new THREE.Box3();
                        sprite.scale.set(100,100,100)
                        console.log(sprite,boxs.getSize(sprite))
                        sprite.position.set(worldPosition.x, worldPosition.y + (boxs.max.y - boxs.min.y), worldPosition.z); // 根据刚才获取的世界坐标设置精灵模型位置，高度加了3，是为了使精灵模型显示在点击模型的上方
