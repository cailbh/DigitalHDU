...
export default {
		...
		methods: {
			...
			getData() {
				const _this = this;
				this.$http
					.get('/api/teachers/getData', { params: { name: "12345" } }, {})
					.then((response) => {
					// 响应成功回调
					// var data = {
					// 	"硕士":200,
					...
					// }
						...
						for(let i in body){
							...
							data[body[i]['position']] =body[i]['num']
						}
							console.log(data)
						var k=0;
						for(let i in data){
			...
			draw() {
				...
				let rSize = (circleCenterX>circleCenterY)?(circleCenterY/2.2):(circleCenterX/2.2)
				var circle = g.append("circle")
					...
					var arcPath = d3.arc()
						.innerRadius(rSize+40)
						.outerRadius(rSize+42);
					var x = circleCenterX
					var y = circleCenterY 
				g.append("path")
