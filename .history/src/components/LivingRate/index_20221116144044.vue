		...
		methods: {
			...
			draw() {
				...
				for(let i in LivingData){
					...
					let jd = {}
					for(let j=0;j<len;j++){
						let val = Math.ceil(Math.random()*300+100)
...
; 
					...
					let cor1 = d3.interpolateRgb(mcolor[5], mcolor[0])(normalize(sum-100))
					let cor2 = d3.interpolateRgb(mcolor[5], mcolor[0])(normalize(sum))
					let cor3 = d3.interpolateRgb(mcolor[5], mcolor[0])(normalize(sum+100))
					let c =  new this.$echarts.graphic.LinearGradient(0, 1, 1, 0, [
					{ offset: 0, color:cor1 },
					{ offset: 0.5, color: cor2 },
					{ offset: 1, color:	cor3 }
					])		

					LivingData[i]['itemStyle']['color'] = c;
				}
							...
							levels: [{}, {
								...
								itemStyle: {
									...
									color:(param)=>{  
										...
										// let cor1 = d3.interpolateRgb(mcolor[5], mcolor[0])(normalize(num-100))
										let cor2 = d3.interpolateRgb(mcolor[5], mcolor[0])(normalize(num))
										// let cor3 = d3.interpolateRgb(mcolor[5], mcolor[0])(normalize(num+100))
										return  new this.$echarts.graphic.LinearGradient(1, 0, 0, 0, [
										{ offset: 0, color:mcolor[5] },
										{ offset: 0.5, color: cor2 },
										{ offset: 1, color:	cor2 }
									])
							// ])
