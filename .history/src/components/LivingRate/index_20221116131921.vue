...
export default {
		...
		methods: {
			...
			draw() {
				...
				var normalize1 = d3.scaleLinear().domain([0, 1000]).range([0, 1]);
				let opData = {
						name: '学院占比',
						itemStyle: {
							color: '#737373'
				...
				for(let i in LivingData){
					...
					for(let j=0;j<len;j++){
						...
						if(jd[idx] !=1 ){
							sum+=val;
							data[idx]['value'] = val;
							let col = d3.interpolateRgb(mcolor[4], mcolor[5])(normalize(val));
							data[idx]['itemStyle']['color'] = col;
							LivingData[i]['children'].push(data[idx]);
						}
						jd[idx] = 1;
					}
					LivingData[i]['value'] = sum;
					let color = d3.interpolateRgb(mcolor[0], mcolor[5])(normalize1(sum));
