...
export default {
	...
	mounted() {		
		console.log(111111111,this.currentProp,JSON.stringify(this.currentProp))	
		console.log(this.currentProp.type)
		this.textData = {
			"home":["学生10000人","教学楼15幢","占地10000亩","宿舍楼78幢"],
			"living":["后勤2000人","食堂楼3幢","占地600亩","宿舍楼78幢"]
		}
		let val = this.currentProp.type
		let _this = this
		if('home'==val){
			_this.title='校园概览'
		}
	...
	methods: {
		...
		draw() {
			let _this = this
			console.log(_this.textData,_this.type)
			let data = _this.textData[_this.type]
			console.log(data)
			let svgWidth = _this.width
			let svgHeight = _this.height
