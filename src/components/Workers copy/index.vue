...
<template>
    <div class="workersContain" id="">
		<div class="ChartHead">校园职工占比</div>
		<div class="ChartBody">
			<div class="workersContain" id="workersContain" ref="wocChart"></div>
		</div>
	</div>
</template>
...
export default {
		...
		mounted() {
			...
			this.height = this.$refs.wocChart.clientHeight
			this.width = this.$refs.wocChart.clientWidth
			this.initSvg()
			this.getData();
			this.draw();
		},
		methods: {
			initSvg(){
				...
				var svg =  d3.select('#workersContain')
					.append("svg")
					.attr("id", "workersSVG")
					.attr("width", svgWidth)
			...
			draw() {
				...
				var svg =  d3.select('#workersSVG')
				svg.select('#workersG').remove()
				var g = svg.append("g")
					.attr("id", "workersG")
					.attr("width", svgWidth)
					.attr("height", svgHeight)
				...
				var circle = g.append("circle")
					...
					.attr("stroke-width", "0")
					.on("mouseover", (d) => {
						d3.select("#cir_totalWorkersNum").attr("r",rSize+10)
					})
					.on("mouseout", (d) => {
						d3.select("#cir_totalWorkersNum").attr("r",rSize)
					})

					// var text = svg.append("text")
				// var textTotal = g.append("text")
				// 	.attr("id", "text_totalWorkersNum")
				// 	// .attr("class", )
